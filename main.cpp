...
int main() {
  ...
  clock_t start, finish;
	...
	int numOfPoints  = 600;
  int numOfQueries = 600;
	int ndim         = 4;

	int binSize      = 20; // bin size : 10~50 (recommanded).
  double distThres = 15;
  ...
  {
    std::vector<double> temp;
    for (int j = 0; j < ndim; j++) temp.push_back(points_vec[i][j]+0.002*(generator()-500));
  ...
  for(int i = 0; i < 100; i++)  tree->kdtree_nearest_neighbor_approximate(points_q_vec, indexVec);
