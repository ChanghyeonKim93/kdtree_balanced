...
double BKDTree::super_key_compare(const double* _point_a, const double* _point_b, const int _cur_dim) {
	...
	for (int i = 0; i < this->nDims; i++) 
	{
		...
		if (diff != 0) break; // ÎßåÏïΩ, ?êÍ∞úÍ∞?Í∞ôÏ? ?äÏúºÎ©?( Ï¶? ?§Î•¥Î©?) Í∑∏ÎÉ• ?®Ïä§ ~
	}
	return diff; // ÎßåÏïΩ, Î™®Îì† Ï∞®Ïõê???´ÏûêÍ∞?Í∞ôÏúºÎ©?0???òÏò§Í≥?/ aÍ∞??¨Î©¥ (+) / bÍ∞??¨Î©¥ (-).
}
...
Node* BKDTree::build_tree_recursively(double*** references, double** temp, const int start, const int end,  const int _depth) {
	...
	if(_depth < this->maxDepth) // If the depth does not reach to max_depth.
	{
		...
		else if (end >  start + 2) // 4Í∞??¥ÏÉÅ ?®Ïïò????		
        {
			//std::cout<<"end > start+2"<<std::endl;
			const int median = start + ( (end - start) / 2); // ?¥Î¶º?úÎã§. ex) (0+5)/2 = 2.5 (x) / 2 (o) , (0+4)/2 = 2
			...
			for (int i = start; i <= end; i++)
			{
				...
				//}
				temp[i] = references[0][i]; //  ?¨Í∏∞Í∞?Î¨∏Ï†ú ?àÏñ¥ Î≥¥ÏûÑ.
			...
			for(int i = 1; i < this->nDims; i++)
			{
				...
				for (int j = start; j <= end; j++)
				{
					...
					if (compare < 0) // ÎßåÏïΩ, references[i][j]Í∞??ÑÏû¨ Ï§ëÏã¨?êÏù∏ node->point Î≥¥Îã§ Î™®Îì† Ï∞®Ïõê?êÏÑú ?ëÎã§Î©? (??Ï∞®Ïõê?êÏÑúÎß??ëÏïÑ??
					{
						//std::cout<<"   down  "<<std::endl;
						references[i - 1][++lower] = references[i][j]; // 
					}
					else if (compare > 0) // ÎßåÏïΩ, references[i][j]Í∞??ÑÏû¨ Ï§ëÏã¨?êÏù∏ node->point Î≥¥Îã§ Î™®Îì† Ï∞®Ïõê?êÏÑú ?¨Îã§Î©? (??Ï∞®Ïõê?êÏÑúÎß?Ïª§ÎèÑ)
					{
	...
	return node; // tree root ÔøΩÔøΩ ÔøΩÔøΩ ÔøΩÔøΩÔøΩÃ¥ÔøΩ.
}
...
Node* BKDTree::create_tree(double** _points) {
	...
	for (int i = 0; i < this->nDims; i++) // ?∏Î¶¨ Íµ¨ÏÑ± Ï§? ?ÑÏ†Ñ ?ôÏùº??????†ú.
	{ 
		end[i] = BKDTree::remove_duplicates(references[i], i);
	}

	for (int i = 0; i < this->nDims - 1; i++) // ÎßåÏïΩ, Î≥ëÌï©?ïÎ†¨ ??Î™®Îì† Ï∞®Ïõê?êÏÑú??endÍ∞?Í∞ôÏ? ?äÏúºÎ©? ?úÎ?Î°?removal???¥Î£®?¥Ï?Ïß??äÏ? Í≤ÉÏù¥ÎØ?°ú ?êÎü¨Î∞úÏÉù.
	{
