...
typedef struct _PointNode {
	double* point;
	int index;
	_PointNode(double* _point, int _index) {
		this->point = _point;
		this->index = _index;
	}
} PointNode;
...
Node* build_tree(double*** references, double** temp, const int start, const int end, const int _ndim, const int _depth, const int _max_depth) {
	...
	if (_depth < _max_depth) { // max_depth ������ node�� �߰��Ѵ�.
		if (end == start) {
...
Node* create_tree(double** _points, const int _npoints, const int _ndim, const int _max_depth) {
	...
	for (int i = 0; i < _ndim; i++) {   // dimension ��ŭ merge �ؼ� references�� �����Ѵ�.
	...
	Node* root = build_tree(references, temp, 0, end[0], _ndim, 0, _max_depth); // ���Ⱑ �����ΰ� ?
