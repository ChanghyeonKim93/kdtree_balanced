...
inline double distance_squared( const vector<double>& a, const vector<double>& b, double thres_near){
	double d = 0;
	double d_spatial = 0;
	double N = a.size();
	...
	if (N == 4) {
		...
		d = d_spatial + 0.7/ 4.0*(a[2] * b[2] + a[3] * b[3] - 1)*(a[2] * b[2] + a[3] * b[3] - 1);
...
class KDTree{
    ...
    public:
		...
		double* thres_near;		  ///< the threshold value for distance ( if the distance exceeds this value, the query becomes invalid point. ) 
...
void KDTree::knn_search( const Point& Xq, int nodeIdx/*=0*/, int dim/*=0*/){
    ...
    // We are in LEAF
    if( node -> isLeaf() ){
        double distance = distance_squared( Xq, points[ node->pIdx ] ,*thres_near);
...
void KDTree::ball_bbox_query(int nodeIdx, Point& pmin, Point& pmax, vector<int>& inrange_idxs, vector<double>& distances, const Point& point, const double& radiusSquared, int dim/*=0*/){
    ...
    if( node->isLeaf() ){
        double distance = distance_squared(points[node->pIdx], point,*thres_near);
