...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]){
	...
	if (!mxIsNumeric(prhs[2]))
		mexErrMsgTxt("varargin{2} must be a distance threshold for nearest matching\n");
	...
	tree->thres_near = mxGetPr(prhs[2]);
	//mexPrintf("tree near threshold : %lf \n", *(tree->thres_near));
    ...
    for(int i=0; i<npoints; i++){
    	for( int j=0; j<ndims; j++ )	query[j]   = query_data[ i+j*npoints ];
    
        int index;
        tree->closest_point(query, index, dists[i]);
		//mexPrintf("tree dist : %lf ,", dists[i]);
